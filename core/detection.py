...
def strip_entropy_single(model, img_raw, clean_dataset_raw, device, transform,
    ...
    Superimposition uses STRAIGHT ADDITION (img + clean, clipped to [0,1]),
    matching the paper's official implementation:
        cv2.addWeighted(background, 1, overlay, 1, 0)
    which computes dst = src1*1.0 + src2*1.0, clipped to [0,255].

    Previous version used a weighted blend (alpha*img + (1-alpha)*clean)
    which halved the trigger signal — for BadNets' small 4x4 patch this
    destroyed STRIP's ability to detect the trigger entirely (TPR = 0%).

    The `alpha` parameter is UNUSED — kept for backward API compatibility.
    ...
    Args:
        ...
        alpha:              UNUSED — kept for backward API compatibility.
                             The paper uses equal-weight addition, not a
                             weighted blend.
    ...
    # Build all N superimposed replicas as raw arrays first, then transform once.
    blended_tensors = []
    for idx in indices:
